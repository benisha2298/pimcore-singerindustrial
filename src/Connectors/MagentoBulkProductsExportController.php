...
class MagentoBulkProductsExportController extends FrontendController
{
	...
	public function defaultAction(Request $request, ApplicationLogger $logger): Response
	{
		...
		$cmd = $php . ' ' . PIMCORE_PROJECT_ROOT . '/bin/console product-data-meg:megexport ' . $datas;
		$process = Process::fromShellCommandline($cmd);
		$process->setTimeout(null);
		$process->run();

		return $this->json(['status' => 'success', 'message' => 'Processed Bulk Products Export to Magento']);
	}
}
