...
class ProductCategoriesExportController extends FrontendController
{
	...
	*/
	public function defaultAction(Request $request, ApplicationLogger $logger): Response
	{
		...
		if ($object->getClassName() == 'CategoryOrTaxonomy') {
			...
			if ($response['status'] == 'success') {
				$logger->info('Categories Export to Woocommerce ', [
					'fileObject'    => new FileObject($this->json($response)),
					'relatedObject' => $object,
					'component'     => 'Product Categories',
					'source'        => 'Categories Export to Woocommerce - ' . $request->get('id'), // optional, if empty, gets automatically filled with 
				]);
			} else {
			...
			if ($response['status'] == 'success') {
				$logger->info('Product Export to Woocommerce ', [
					'fileObject'    => new FileObject($this->json($response)),
					'relatedObject' => $object,
					'component'     => 'Product Export',
					'source'        => 'Product Export to Woocommerce - ' . $request->get('id'), // optional, if empty, gets automatically filled with 
				]);
			} else {
				$logger->error('Product Export to Woocommerce ', [
					'fileObject'    => new FileObject($this->json($response)),
					'relatedObject' => $object,
					'component'     => 'Product Export',
					'source'        => 'Product Export to Woocommerce - ' . $request->get('id'), // optional, if empty, gets automatically filled with 
	...
	public function CategoriesExport($id)
		{
			...
			if($success == 1){
				return ['status' => 'success', 'message' => 'Category exported to AltiusNxt woocommerce', 'query' => $category_data_final];
			}elseif($success == 2) {
				$category_data->setLastUpdated(\Carbon\Carbon::now());
				$category_data->setLastWoocommerceStatus('Failed');
				$category_data->save();
				return ['status' => 'failure', 'message' => 'Category already exist in AltiusNxt woocommerce', 'query' => array()];
			}elseif($success == 3){
				$category_data->setLastUpdated(\Carbon\Carbon::now());
				$category_data->setLastWoocommerceStatus('Failed');
				$category_data->save();
				return ['status' => 'failure', 'message' => 'Parent Category not exist in AltiusNxt woocommerce', 'query' => array()];
			}else{
				$category_data->setLastUpdated(\Carbon\Carbon::now());
				$category_data->setLastWoocommerceStatus('Failed');
				$category_data->save();
	...
	public function ProductsExportToWoocommerce($id)
	{
		...
		// Update the condition for exact match
		$details = $category_listing->setCondition("CategoryName = ?", [$pim_categories]);
			...
			foreach ($details as $sep_category) {
				if ($sep_category->getWoocommerceCategoryId() != 0 && $sep_category->getWoocommerceCategoryId() != '') {
					...
					if ($sep_category->getWoocommerceCategoryParentId() != 0 && $sep_category->getWoocommerceCategoryParentId() != '') {
						$parent_category_data = $category_listing->setCondition("CategoryName = ?", [$sep_category->getParentCategory()]);
						if (!empty($parent_category_data)) {
							foreach ($parent_category_data as $parent_category) {
								...
								if (!empty($parent_category->getParentCategory())) {
									$parent_category_data = $category_listing->setCondition("CategoryName = ?", [$parent_category->getParentCategory()]);
									if (!empty($parent_category_data)) {
										foreach ($parent_category_data as $parent_category) {
											$cat_arr = array(
												'id' => (int)$parent_category->getWoocommerceCategoryId(),
		...
		$success = 0;
		$product_id = $product_data->getWoocommerceProductId();
		if(!empty($product_id)) {
			$product_array = array(
			...
			if(!empty($create_products->id)){
				$success_product[] = $create_products;
				$product_data->setWoocommerceProductId($create_products->id);
				$product_data->setLastUpdated(\Carbon\Carbon::now());
				$product_data->setLastWoocommerceStatus('Success');
				$product_data->save();
				$success = 1;
