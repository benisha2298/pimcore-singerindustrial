...
class MagentoProductsConnectorController extends FrontendController
{
	...
	public function ProductsExportToMagento($id)
	{
		...
		$attributesArray = [
			...
			[
				...
				'attribute_code' => 'upc',
				'position' => 4,
				'visible' => !empty($product_data->getUPC()) ? true : false,
				'variation' => false,
		...
		// Assuming $attributesArray is an array containing attributes with their values
		foreach ($attributesArray as $attribute_key => $attribute_value) {
			if (!empty($attribute_value['attribute_value'])) {
				...
				// Check if the attribute already exists in Magento
				$endpoint = "/products/attributes/" . $attribute_code;
				$method = 'GET';
				$api_call = $this->MagentoApiCall($endpoint, $method, $access_token, []);
				
				// If the attribute doesn't exist, create it with options
				if (empty($api_call->attribute_code)) {
					$add_options = [];
					
					// Only add an option if the attribute value is not empty
					if (!empty($attribute_value['attribute_value'])) {
						$add_options[] = [
					...
					// Prepare the attribute data to send
					$attribute_array = [
						"attribute" => [
							"attribute_code" => $attribute_code,
					...
					// Call Magento API to create the attribute
					$endpoint = '/products/attributes';
					$method = 'POST';
					$api_call = $this->MagentoApiCall($endpoint, $method, $access_token, $attribute_array);
		
					// If the attribute is created, retrieve its options
					if (!empty($api_call->attribute_code)) {
						...
						if (!empty($created_options)) {
							// Loop through created options and set them as custom attributes, using value (ID)
							foreach ($created_options as $option_val) {
								$selected_option_id = $option_val->value; // Value ID instead of label
								$custom_attributes[] = [
									'attribute_code' => $attribute_code,
									'value' => $selected_option_id // Use option ID (value)
								];
							}
						}
					}
				} else {
					// If the attribute exists, fetch its options
					...
					if (!empty($options_api_call)) {
						// Check if the requested attribute value is already an option
						$found_option = false;
						foreach ($options_api_call as $opt_value) {
							if ($opt_value->label == $attribute_value['attribute_value']) {
								$selected_option_id = $opt_value->value; // Option value ID
								$custom_attributes[] = [
									'attribute_code' => $attribute_code,
									'value' => $selected_option_id // Use value ID here
								];
								$found_option = true;
								break;
							}
						...
						if (!$found_option) {
							...
							if (!empty($new_option_api_call->option_id)) {
								$selected_option_id = $new_option_api_call->option_id; // Option value ID
								$custom_attributes[] = [
									'attribute_code' => $attribute_code,
									'value' => $selected_option_id // Use value ID here
								];
		...
		// return ['status' => 'success', 'message' => $attri_response];
