...
class MagentoProductExportCommand extends AbstractCommand
{
	private $logger;
    ...
    {
        $this
            ->setName('product-data-meg:megexport')
    ...
    public function importProducts($id)
    {
		...
		$api_call = $this->MagentoApiCall($endpoint, $method, $access_token, array());
            // return $this->json(['status' => 'success', 'message' => $api_call]);
            $attribute_set_id = '';
            ...
            if (!empty($magento_product_sku) || !empty($dass_magento_product_sku)) {
                ...
                $api_call = $this->MagentoApiCall($endpoint, $method, $access_token, $product_array);
                $success_product[] = $api_call;
                $this->logger->info('Plumbing and Safety Export to Magento ', [
                    'fileObject'    => new FileObject(json_encode($api_call)),
                    'relatedObject' => $product_data,
                    'component'     => 'Plumbing & Safety Products',
                    'source'        => 'Plumbing and Safety Export to Magento'. json_encode($api_call), // optional, if empty, gets automatically filled with 
                ]);
                // Handle the response from the API call
                if (!empty($api_call->id)) {
                    $success_product[] = $api_call;
