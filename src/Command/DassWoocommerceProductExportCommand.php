...
class DassWoocommerceProductExportCommand extends AbstractCommand
{
	private $logger;
    ...
    {
        $this
            ->setName('dass-data:dasswooexport')
        ...
        foreach ($batch as $id) {
            ...
            if (!$response) {
				...
				$this->logger->info('Successfully imported product ', [
					...
					'source'        => 'Plumbing and Safety Export to Dass Woocommerce'. json_encode($response), // optional, if empty, gets automatically filled with 
				]);
            }
        }
    }
    ...
    public function importProducts($id)
    {
        ...
        $endpoint = $product_data->getDassWoocommerceProductId() ? '/products/' . $product_data->getDassWoocommerceProductId() : '/products';
        $method = $product_data->getDassWoocommerceProductId() ? 'PUT' : 'POST';
        ...
        if (isset($response->id)) {
            $product_data->setDassWoocommerceProductId($response->id);
    ...
    private function getCategories($pim_categories)
    {
        ...
        foreach ($details as $sep_category) {
            if ($sep_category->getDassWoocommerceCategoryId()) {
                $cat_arr = [
                ...
                if ($sep_category->getDassWoocommerceCategoryParentId()) {
